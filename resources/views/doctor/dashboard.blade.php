<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <p class="vethub-breadcrumb">Home / Clinical Dashboard</p>
        </div>
    </x-slot>
    ...
    <div class="stat-card-grid" style="margin-bottom:24px;">
        <div class="stat-card-vethub">
            ...
            <a href="{{ route('doctor.appointments.index') }}" style="font-size:12px;color:var(--primary);text-decoration:none;">View Queue →</a>
        </div>
        <div class="stat-card-vethub">
            <div class="stat-title">Vet Clinic</div>
            <div class="stat-value">{{ $vet_count }}</div>
            <span style="font-size:12px;color:var(--text-muted);">Scheduled today</span>
        </div>
        <div class="stat-card-vethub">
            <div class="stat-title">Grooming</div>
            <div class="stat-value">{{ $grooming_count }}</div>
            <span style="font-size:12px;color:var(--text-muted);">Active sessions</span>
        </div>
        <div class="stat-card-vethub">
            <div class="stat-title">Pet Hotel</div>
            <div class="stat-value">{{ $hotel_count }}</div>
            <span style="font-size:12px;color:var(--text-muted);">Currently booked</span>
        </div>
    </div>

    <!-- Quick Actions -->
    <div class="stat-card-vethub" style="margin-bottom:24px;">
        <h4 style="font-size:16px;font-weight:600;margin:0 0 16px;">Quick Actions</h4>
