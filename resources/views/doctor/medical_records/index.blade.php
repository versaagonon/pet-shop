                    ...
                    <ol class="inline-flex items-center space-x-1 md:space-x-3">
                        ...
                        <li>
                            <div class="flex items-center">
                                ...
                                <span class="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Rekam Medis</span>
                            </div>
                        </li>
                    </ol>
                </nav>
                <h2 class="text-2xl font-bold text-slate-800">Log Klinis</h2>
            </div>
            <a href="{{ route('doctor.medical-records.create') }}" class="btn-mewah">
                ...
                <tbody>
                    @forelse($medicalRecords as $record)
                        <tr class="hover:bg-slate-50 transition-colors">
                            <td>
                                <span class="font-bold text-slate-700 leading-tight">{{ \Carbon\Carbon::parse($record->date)->format('d M Y') }}</span>
                            </td>
                            <td>
                                <div class="flex items-center gap-2">
                                    <div class="w-1.5 h-1.5 rounded-full bg-rose-400"></div>
                                    <span class="font-bold text-slate-700">{{ $record->pet->name }}</span>
                                </div>
                            </td>
                            <td>
                                <div class="max-w-xs truncate text-slate-500 font-medium text-sm" title="{{ $record->diagnosis }}">
                                    {{ Str::limit($record->diagnosis, 60) }}
                                </div>
                            </td>
                            <td>
                                <div class="flex items-center gap-2">
                                    <div class="w-6 h-6 bg-blue-50 rounded text-blue-500 flex items-center justify-center text-[10px] font-bold">DR</div>
                                    <span class="text-xs font-bold text-slate-600">{{ $record->user->name }}</span>
                                </div>
                            </td>
                            <td class="text-right">
                                <div class="flex justify-end gap-2">
                                    <a href="{{ route('doctor.medical-records.edit', $record) }}" class="p-2 text-slate-400 hover:text-blue-500 hover:bg-blue-50 rounded-lg transition-all">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                    </a>
                                    <form action="{{ route('doctor.medical-records.destroy', $record) }}" method="POST" onsubmit="return confirm('Hapus entri catatan ini?')">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="py-16 text-center text-slate-300">
                                <p class="font-bold text-sm uppercase tracking-widest">Rekam Medis Tidak Ditemukan</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
