                    ...
                    <ol class="inline-flex items-center space-x-1 md:space-x-3">
                        ...
                        <li>
                            <div class="flex items-center">
                                ...
                                <span class="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Riwayat</span>
                            </div>
                        </li>
                    </ol>
                </nav>
                <h2 class="text-2xl font-bold text-slate-800">Riwayat Keuangan</h2>
            </div>
                ...
                <tbody>
                    @forelse($invoices as $invoice)
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td>
                                <div class="flex items-center gap-1.5 px-2.5 py-1 rounded-full border w-fit font-bold uppercase tracking-widest text-[9px] {{ $invoice->status === 'paid' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-rose-50 text-rose-600 border-rose-100' }}">
                                    <div class="w-1 h-1 rounded-full bg-current"></div>
                                    {{ $invoice->status === 'paid' ? 'DIBAYAR' : 'BELUM DIBAYAR' }}
                                </div>
                            </td>
                            <td class="text-right">
                                <div class="flex justify-end gap-2 text-right">
                                    <a href="{{ route('admin.invoices.show', $invoice) }}" class="p-2 text-slate-400 hover:text-blue-500 hover:bg-blue-50 rounded-lg transition-all" title="Lihat Transkrip">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="py-16 text-center text-slate-300">
                                <p class="font-bold text-sm uppercase tracking-widest">Data Riwayat Tidak Ditemukan</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
