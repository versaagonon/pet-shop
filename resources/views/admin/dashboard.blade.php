<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <p class="vethub-breadcrumb">Home / Dashboard</p>
        </div>
    </x-slot>
    ...
    <div class="stat-card-grid" style="margin-bottom:24px;">
        <div class="stat-card-vethub">
            ...
            <a href="{{ route('admin.invoices.index') }}" style="font-size:12px;color:var(--primary);text-decoration:none;">View Details →</a>
        </div>
        <div class="stat-card-vethub">
        ...
        <div class="stat-card-vethub">
            <div class="stat-title">New Owners</div>
            <div class="stat-value">{{ $new_owners_count }}</div>
            <span style="font-size:12px;color:var(--text-muted);">This month</span>
        </div>
        <div class="stat-card-vethub">
            <div class="stat-title">New Pets</div>
    ...
    <div style="display:grid;grid-template-columns:2fr 1fr;gap:20px;">
        <div class="stat-card-vethub">
            <h4 style="font-size:16px;font-weight:600;margin:0 0 16px;">Client Growth Summary</h4>
            <div style="height:280px;">
                <canvas id="growthChart"></canvas>
            </div>
        </div>
        <div class="stat-card-vethub">
            <h4 style="font-size:16px;font-weight:600;margin:0 0 16px;">Recent Activity</h4>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        new Chart(document.getElementById('growthChart').getContext('2d'), {
            ...
            data: {
                ...
                datasets: [
                    { label: 'Owners', data: {!! json_encode($owner_growth) !!}, borderColor: '#3b82f6', backgroundColor: 'rgba(59,130,246,0.08)', fill: true, tension: 0.4, pointRadius: 3, borderWidth: 2 },
                    { label: 'Pets', data: {!! json_encode($pet_growth) !!}, borderColor: '#ef4444', backgroundColor: 'rgba(239,68,68,0.05)', fill: true, tension: 0.4, pointRadius: 3, borderWidth: 2 }
                ]
            },
            options: {
