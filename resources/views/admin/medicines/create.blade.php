<x-app-layout>
    <x-slot name="header">
        <div>
            <h2 style="font-size:22px;font-weight:700;margin:0;">{{ isset($medicine) ? 'Ubah Obat Campuran' : 'Tambah Obat Campuran' }}</h2>
            <p class="vethub-breadcrumb">Obat Campuran / {{ isset($medicine) ? 'Ubah' : 'Tambah Baru' }}</p>
        </div>
    </x-slot>
            ...
            <form action="{{ isset($medicine) ? route('admin.medicines.update', $medicine) : route('admin.medicines.store') }}" method="POST" onsubmit="return buildCompositionsJson()">
                ...
                <div style="display:flex;flex-direction:column;gap:16px;">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ $medicine->name ?? old('name') }}" placeholder="cth. grooming, antiparasit" required>
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="2" placeholder="Catatan opsional">{{ $medicine->description ?? old('description') }}</textarea>
                    </div>

                    <!-- Compositions Section -->
                    <div style="border-top:1px solid var(--border-light);padding-top:16px;">
                        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px;">
                            <label style="margin:0;font-size:14px;font-weight:700;">🧪 Komposisi</label>
                            <button type="button" id="addCompBtn" class="btn-outline" style="padding:4px 12px;font-size:12px;">+ Tambah Komposisi</button>
                        </div>
                        <div id="comp-rows">
                            @php
                                $existingComps = [];
                        ...
                        <div id="no-comp-msg" style="text-align:center;padding:12px;color:var(--text-muted);font-size:13px;{{ count($existingComps) > 0 ? 'display:none;' : '' }}">
                            Belum ada komposisi yang ditambahkan.
                        </div>
                ...
                <div style="display:flex;justify-content:flex-end;gap:12px;margin-top:24px;padding-top:16px;border-top:1px solid var(--border-light);">
                    <a href="{{ route('admin.medicines.index') }}" class="btn-outline">Batal</a>
                    <button type="submit" class="btn-primary">{{ isset($medicine) ? 'Simpan Perubahan' : 'Tambah Obat Campuran' }}</button>
                </div>
            </form>
        </div>
    </div>
