<x-app-layout>
    <x-slot name="header">
        <div>
            <h2 style="font-size:22px;font-weight:700;margin:0;">{{ isset($product) ? 'Ubah Produk' : 'Tambah Produk' }}</h2>
            <p class="vethub-breadcrumb">Produk / {{ isset($product) ? 'Ubah' : 'Tambah Baru' }}</p>
        </div>
    </x-slot>
            ...
            <form action="{{ isset($product) ? route('admin.products.update', $product) : route('admin.products.store') }}" method="POST">
                ...
                <div style="display:flex;flex-direction:column;gap:16px;">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ $product->name ?? old('name') }}" placeholder="cth. Obat Cacing Kucing" required>
